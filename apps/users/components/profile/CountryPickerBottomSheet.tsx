...
/** Convert ISO alpha-2 code to flag emoji (works on iOS/Android/web). */
...
/**
 * Searchable country picker rendered as a @gorhom/bottom-sheet modal.
 * Each row shows a Unicode flag emoji, country name, and dial code.
 */
    ...
    ({ item }: { item: Country }) => {
      ...
      return (
        <Pressable
          ...
          <Text style={[styles.dialCode, { color: theme.textMuted }]}>{item.dialCode}</Text>
          {isSelected ? (
            <Ionicons name='checkmark' size={18} color={theme.accent} style={styles.check} />
          ) : null}
        </Pressable>
      );
    },
    [selected, theme, onSelect],
  );
  ...
  return (
    <BottomSheetModal
      ...
      {/*
        ...
        BottomSheetView — so the sheet's layout engine can give it a
        bounded height and make it scroll correctly.
      */}
      <BottomSheetView style={[styles.stickyHeader, { backgroundColor: theme.card, borderBottomColor: theme.divider }]}>
        <Text style={[styles.title, { color: theme.textPrimary, fontFamily: fonts.semiBold }]}>
          Select Country
        </Text>
        <View style={[styles.searchWrap, { backgroundColor: theme.surfaceMuted, borderColor: theme.divider }]}>
      ...
      <BottomSheetFlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            <Text style={[styles.emptyText, { color: theme.textMuted }]}>No results for "{query}"</Text>
          </View>
        }
      />
    </BottomSheetModal>
